...
import { MessageSquare, LogIn, LogOut, Plus, Users, UserCircle2 } from 'lucide-react';
...
export function Navbar() {
  ...
  const navItems = [
    ...
    { name: 'Profile', path: user ? `/profile/${user.uid}` : '/community', icon: UserCircle2 },
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        <div className="flex items-center gap-8">
          <Link to="/" className="flex items-center gap-2 group">
            ...
            <span className="text-xl font-bold tracking-tight text-emerald-500 font-mono italic">GitTalk</span>
          </Link>
              ...
              <div className="flex items-center gap-3 pl-4 border-l border-zinc-800">
                <Link to={`/profile/${user.uid}`} className="flex items-center gap-2 group">
                  <img
                    src={user.photoURL || ''}
                    alt=""
                    className="w-8 h-8 rounded-full border border-zinc-700 group-hover:border-emerald-500 transition-colors"
                  />
